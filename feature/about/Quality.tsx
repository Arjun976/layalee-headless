...
export default function AboutQuality({ qualityData }: AboutQualityProps) {
  const enabled = qualityData?.enabled !== false;
  ...
  const title = qualityData?.title || 'Quality You Can Trust';
  const paragraphs = qualityData?.paragraphs && qualityData.paragraphs.length > 0
    ? qualityData.paragraphs.map(p => p.text)
    : ['Every planter is selected with a focus on performance, longevity, and design excellence.'];

  // Resolve Features List
  const specsList = qualityData?.features && qualityData.features.length > 0
    ? qualityData.features.map(f => ({
        title: f.title || '',
        desc: f.description || '',
        icon: f.icon?.url ? (
  ...
  const sideImage = qualityData?.image?.url || '/quality_img.png';
      ...
      <div className="w-full xl:w-[45%] flex flex-col justify-center py-16 xl:py-24 px-5 md:px-[30px] xl:pl-20 xl:pr-10">
        <div className="flex flex-col gap-10 max-w-[674px]">
          ...
          <div className="flex flex-col gap-4 text-left">
            ...
            <div className="text-[#F5F3EF]/85 font-['Google_Sans',sans-serif] text-base md:text-[18px] leading-relaxed flex flex-col gap-2">
              {paragraphs.map((p, i) => (
                <p key={i}>{p}</p>
              ))}
            </div>
